...
const DEFAULT_OAUTH_PROTOCOL_BY_CHANNEL = {
  development: "dictatekit-dev",
  staging: "dictatekit-staging",
  production: "dictatekit",
};
const BASE_WINDOWS_APP_ID = "com.dictatekit.app";
...
function resolveAppChannel() {
  const rawChannel = (process.env.DICTATEKIT_CHANNEL || process.env.VITE_DICTATEKIT_CHANNEL || "")
    .trim()
    .toLowerCase();
...
process.env.DICTATEKIT_CHANNEL = APP_CHANNEL;
...
function configureChannelUserDataPath() {
  ...
  const isolatedPath = path.join(app.getPath("appData"), `DictateKit-${APP_CHANNEL}`);
...
if (process.platform === "linux") {
  app.setDesktopName("dictatekit.desktop");
}
...
function getOAuthProtocol() {
  const fromEnv = (process.env.VITE_DICTATEKIT_PROTOCOL || process.env.DICTATEKIT_PROTOCOL || "")
    .trim()
    .toLowerCase();
...
// True source of truth for whether dictatekit:// resolves on Linux — the same
...
function registerDictateKitProtocol() {
...
const protocolRegistered = registerDictateKitProtocol() || isOAuthSchemeRegistered();
...
if (process.platform === "darwin" && app.getName() !== "DictateKit") {
  app.setName("DictateKit");
}
...
function parseAuthBridgePort() {
  const raw = (process.env.DICTATEKIT_AUTH_BRIDGE_PORT || "").trim();
...
function getOauthCookieName() {
  return process.env.NODE_ENV === "production"
    ? "__Secure-dictatekit.session_token"
    : "dictatekit.session_token";
}
...
function startAuthBridgeServer() {
  ...
  authBridgeServer = http.createServer(async (req, res) => {
    ...
    res.end(
      "<html><body><h3>DictateKit sign-in complete.</h3><p>You can close this tab.</p></body></html>"
    );
  });
...
async function startApp() {
  ...
  // calls to DictateKit's auth and API hosts are treated as same-origin.
