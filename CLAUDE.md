# DictateKit Technical Reference for AI Assistants

This document provides comprehensive technical details about the DictateKit project architecture for AI assistants working on the codebase.

## Project Overview

DictateKit is an Electron-based desktop dictation application that uses whisper.cpp for speech-to-text transcription. It supports both local (privacy-focused) and cloud (OpenAI API) processing modes.
...
- **hyprlandShortcut.js**: Hyprland Wayland global shortcut integration
  - Uses D-Bus service to receive hotkey toggle commands (same `com.dictatekit.App` service)
...
- **cliBridge.js**: Loopback HTTP server on ports 8200–8219, bearer-token auth (token at `~/.dictatekit/cli-bridge.json`), 127.0.0.1-only. Used by the unified CLI to talk to a running desktop app.
...
- **ReasoningService.ts**: AI processing for agent-addressed commands
  ...
  - Provider implementations live in a registry at `src/services/ai/inferenceProviders/index.ts` covering 8 providers (`anthropic`, `enterprise`, `gemini`, `groq`, `lan`, `local`, `openai`, `dictatekit`), each implementing the `InferenceProvider` interface from `types.ts`
...
- **whisper.js**: Native binary wrapper for local transcription
  ...
  - Models stored in `~/.cache/dictatekit/whisper-models/`
...
- **parakeet.js**: Model management for NVIDIA Parakeet ASR models
  ...
  - Models stored in `~/.cache/dictatekit/parakeet-models/`
...
- Qdrant data: `~/.cache/dictatekit/qdrant-data/`
- Qdrant binary: `resources/bin/qdrant-{platform}-{arch}` (bundled — downloaded during `prebuild` / `predev`)
- Embedding model: `~/.cache/dictatekit/embedding-models/all-MiniLM-L6-v2/` (auto-downloaded on first launch)
...
Enable with `--log-level=debug` or `DICTATEKIT_LOG_LEVEL=debug` (can be set in `.env`):
...
On GNOME Wayland, Electron's `globalShortcut` API doesn't work due to Wayland's security model. DictateKit uses native GNOME shortcuts:
...
3. `gnomeShortcut.js` creates D-Bus service at `com.dictatekit.App`
...
- D-Bus service: `com.dictatekit.App`
- D-Bus path: `/com/dictatekit/App`
- gsettings path: `/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/dictatekit/`
...
On Hyprland (wlroots Wayland compositor), Electron's `globalShortcut` API and the `GlobalShortcutsPortal` feature don't work reliably. DictateKit uses native Hyprland keybindings:
...
3. `hyprlandShortcut.js` creates D-Bus service at `com.dictatekit.App` (same as GNOME)
...
4. Do NOT translate: brand names (DictateKit, Pro), technical terms (Markdown, Signal ID), format names (MP3, WAV), AI system prompts
...
7. **Local Semantic Search Not Working**:
   ...
   - Embedding model should be in `~/.cache/dictatekit/embedding-models/all-MiniLM-L6-v2/model.onnx` (auto-downloaded on first app launch)
