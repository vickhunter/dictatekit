#!/bin/bash
# Post-install script for DictateKit (deb/rpm)
...
CHROME_SANDBOX=$(dpkg -L dictatekit 2>/dev/null | grep chrome-sandbox || echo "")
if [ -z "$CHROME_SANDBOX" ]; then
  # Fallback: conventional electron-builder install path
  CHROME_SANDBOX="/opt/DictateKit/chrome-sandbox"
fi
