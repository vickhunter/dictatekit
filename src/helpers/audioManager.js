...
`;
  ...
  async processAudio(audioBlob, metadata = {}) {
    ...
    try {
      ...
      const isDictateKitCloudMode = !useLocalWhisper && cloudTranscriptionMode === "dictatekit";
      const useCloud = isDictateKitCloudMode && isSignedIn;
      logger.debug(
      ...
      } else if (isDictateKitCloudMode) {
        if (!isSignedIn) {
          const err = new Error(
            "DictateKit Cloud requires sign-in. Please sign in again or switch to BYOK mode."
          );
          err.code = "AUTH_REQUIRED";
          err.messageKey = "hooks.audioRecording.errorDescriptions.sessionExpired";
          throw err;
        }
        activeModel = "dictatekit-cloud";
        result = await this.processWithDictateKitCloud(audioBlob, metadata);
      } else {
  ...
  async runTranslationChain({ text, settings, agentName, route, cleanup }) {
    ...
    try {
      return await executeTranslationChain({
        ...
        translateIsCloud: route.config?.provider === "dictatekit",
  ...
  async processWithDictateKitCloud(audioBlob, metadata = {}) {
    if (!navigator.onLine) {
    ...
    const cleanupCloudMode = settings.cleanupCloudMode || "dictatekit";
    if (
      (settings.useCleanupModel && !this.skipReasoning && cleanupCloudMode === "dictatekit") ||
      (this.translationRequested &&
        !this.skipReasoning &&
    ...
    if (processedText && !this.skipReasoning) {
      ...
      try {
        ...
        } else if (route.kind === "cleanup" && cleanupCloudMode === "dictatekit") {
          const reasonResult = await withSessionRefresh(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) err.code = "INVALID_KEY";
          ...
          // The user's own provider rate-limited the request — not an DictateKit plan limit
  ...
  shouldUseStreaming(isSignedInOverride) {
    ...
    // Corti (BYOK) streams over its own WSS — independent of DictateKit Cloud.
    if (s.cloudTranscriptionProvider === "corti" && s.cloudTranscriptionMode === "byok") {
      return !!(s.cortiClientId && s.cortiClientSecret);
    }

    // Tinfoil realtime streams without an DictateKit account.
    if (s.cloudTranscriptionProvider === "tinfoil") {
    ...
    if (REALTIME_MODELS.has(s.cloudTranscriptionModel)) {
      ...
      if (s.cloudTranscriptionMode === "dictatekit") return !!(isSignedInOverride ?? s.isSignedIn);
      return false;
    }

    if (s.cloudTranscriptionMode !== "dictatekit" || !(isSignedInOverride ?? s.isSignedIn)) {
      return false;
    }
      ...
      const [, wsResult] = await Promise.all([
        ...
        withSessionRefresh(async () => {
          ...
          const res = await provider.warmup({
            ...
            mode: cloudTranscriptionMode === "byok" ? "byok" : "dictatekit",
  ...
  async startStreamingRecording(forceDefaultMic = false) {
    try {
      ...
      if (error.name === "NotAllowedError" || error.name === "PermissionDeniedError") {
        ...
        errorDescription =
          "Your DictateKit Cloud session is unavailable. Please sign in again from Settings.";
  ...
  async stopStreamingRecording() {
    ...
    if (finalText && !this.skipReasoning) {
      ...
      const cleanupCloudMode = stSettings.cleanupCloudMode || "dictatekit";
    ...
    if (!finalText && durationSeconds > 2 && fallbackBlob?.size > 0) {
      ...
      if (target === "skip") {
        logger.warn(
          "Skipping batch fallback: DictateKit Cloud session signed out",
          {},
          "streaming"
        );
      } else {
        ...
        try {
          ...
          const batchResult =
            target === "cloud"
              ? await this.processWithDictateKitCloud(fallbackBlob, { durationSeconds })
