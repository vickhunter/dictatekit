...
const COMPONENT_NAME = "dictatekit";
...
class KDEShortcutManager {
  ...
  _findCallbackByFriendlyName(name) {
    ...
    for (const slotName of this.registeredSlots) {
      friendlyToSlot[`DictateKit ${slotName}`] = slotName;
      friendlyToSlot[`DictateKit`] = "dictation"; // legacy compat
    }
  ...
  async registerKeybinding(electronHotkey, slotName = "dictation", callback) {
    ...
    const actionId = [COMPONENT_NAME, slotName, "DictateKit", `DictateKit ${slotName}`];
