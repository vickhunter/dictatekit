...
import { DICTATEKIT_API_URL } from "../config/constants";
...
export const authClient = createAuthClient({
  ...
  fetchOptions: {
    ...
    headers: { "x-dictatekit-source": "desktop" },
...
const LAST_SIGN_IN_STORAGE_KEY = "dictatekit:lastSignInTime";
...
export async function deleteAccount(): Promise<{ error?: Error }> {
  if (!DICTATEKIT_API_URL) {
    return { error: new Error("API not configured") };
  }

  try {
    const res = await fetch(`${DICTATEKIT_API_URL}/api/auth/delete-account`, {
      method: "DELETE",
      credentials: "include",
    });
...
export async function signInWithSocial(provider: SocialProvider): Promise<{ error?: Error }> {
  try {
    ...
    if (isElectron) {
      ...
      const protocol = (await window.electronAPI?.getOAuthProtocol?.()) || "dictatekit";
