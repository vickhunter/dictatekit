...
function TranscriptionSection({
  ...
  const handleTranscriptionModeSelect = (mode: InferenceMode) => {
    ...
    setCloudTranscriptionMode(mode === "dictatekit" ? "dictatekit" : "byok");

    const toastKey = {
      dictatekit: "switchedCloud",
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (anyFailed) {
            ...
            window.dispatchEvent(new Event("dictatekit-models-cleared"));
            showAlertDialog({
        ...
        return (
          <div className="space-y-5">
            {!AUTH_URL ? (
              ...
              <>
                {/* Plan comparison + checkout: advertising, off in this fork. */}
                {PROMOTIONS_ENABLED && (
                  <>
                      ...
                      <div className="rounded-md border-2 border-primary/50 bg-primary/8 dark:border-primary/40 dark:bg-primary/10 p-2.5 flex flex-col relative">
                        ...
                        {usage?.isSubscribed && usage?.plan === "business" && !usage?.isTrial ? (
                          ...
                          <Button
                            onClick={() =>
                              handleSwitchPlan(
                                billingState.business ? "annual" : "monthly",
                                "business"
                              )
                            }
                            disabled={previewLoading || usage.checkoutLoading}
                      ...
                      steps: [
                        ...
                        },
                        {
                          ...
                          desc: t("settingsPage.general.waylandPaste.guide.group.step2Desc", {
                            defaultValue:
                              "Group changes only take effect after a new login session. Log out of your desktop and log back in, then reopen DictateKit.",
                          }),
                        },
                      ],
                    },
