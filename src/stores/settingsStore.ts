...
function migrateProviderSettings() {
  ...
  let newReasoningMode: InferenceMode = "dictatekit";
...
// so nothing is copied and the upload context falls through to its DictateKit
// Cloud defaults.
...
function migrateAgentMode() {
  ...
  let agentInferenceMode: InferenceMode = "dictatekit";
...
export const useSettingsStore = create<SettingsState>()((set, get) => ({
  ...
  cloudTranscriptionMode: readString("cloudTranscriptionMode", "dictatekit"),
  cleanupCloudMode: readString("cleanupCloudMode", "dictatekit"),
  ...
  transcriptionMode: (() => {
    const v = readString("transcriptionMode", "dictatekit");
    if (v === "dictatekit" || v === "providers" || v === "local" || v === "self-hosted") return v;
    return "dictatekit" as InferenceMode;
  })(),
  ...
  cleanupMode: (() => {
    const v = readString("cleanupMode", "dictatekit");
    if (
      v === "dictatekit" ||
      v === "providers" ||
      v === "local" ||
  ...
  meetingTranscriptionMode: (() => {
    const v = readString("meetingTranscriptionMode", "dictatekit");
    if (v === "dictatekit" || v === "providers" || v === "local" || v === "self-hosted") return v;
    return "dictatekit" as InferenceMode;
  })(),
  ...
  uploadTranscriptionMode: (() => {
    const v = readString("uploadTranscriptionMode", "dictatekit");
    if (v === "dictatekit" || v === "providers" || v === "local" || v === "self-hosted") return v;
    return "dictatekit" as InferenceMode;
  })(),
  ...
  noteFormattingMode: (() => {
    const v = readString("noteFormattingMode", "dictatekit");
    if (
      v === "dictatekit" ||
      v === "providers" ||
      v === "local" ||
  ...
  translationMode: (() => {
    const v = readString("translationMode", "dictatekit");
    if (
      v === "dictatekit" ||
      v === "providers" ||
      v === "local" ||
  ...
  translationCloudMode: readString("translationCloudMode", "dictatekit"),
  ...
  chatAgentCloudMode: readString("chatAgentCloudMode", "dictatekit"),
  chatAgentMode: (() => {
    const v = readString("chatAgentMode", "dictatekit");
    if (
      v === "dictatekit" ||
      v === "providers" ||
      v === "local" ||
  ...
  dictationAgentMode: (() => {
    const v = readString("dictationAgentMode", "dictatekit");
    if (
      v === "dictatekit" ||
      v === "providers" ||
      v === "local" ||
  ...
  dictationAgentCloudMode: readString("dictationAgentCloudMode", "dictatekit"),
  ...
  setCloudTranscriptionForAllScopes: (settings: Partial<TranscriptionSettings>) => {
    ...
    // scope — otherwise the UI keeps showing the previous mode (e.g. DictateKit
...
export const selectIsCloudCleanupMode = (state: SettingsState) =>
  state.isSignedIn && state.cleanupMode === "dictatekit" && state.cleanupCloudMode === "dictatekit";

export const selectEffectiveCleanupProvider = (state: SettingsState) =>
  selectIsCloudCleanupMode(state) ? "dictatekit" : state.cleanupProvider;

export const selectIsCloudChatAgentMode = (state: SettingsState) =>
  state.isSignedIn &&
  state.chatAgentMode === "dictatekit" &&
  state.chatAgentCloudMode === "dictatekit";

export const selectIsCloudDictationAgentMode = (state: SettingsState) =>
  state.isSignedIn &&
  state.dictationAgentMode === "dictatekit" &&
  state.dictationAgentCloudMode === "dictatekit";

export const selectIsCloudTranslationMode = (state: SettingsState) =>
  state.isSignedIn &&
  state.translationMode === "dictatekit" &&
  state.translationCloudMode === "dictatekit";

export const selectIsCloudNoteFormattingMode = (state: SettingsState) => {
  const cfg = selectResolvedNoteFormatting(state);
  return state.isSignedIn && cfg.mode === "dictatekit" && cfg.cloudMode === "dictatekit";
};
