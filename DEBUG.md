...
```bash
# macOS
/Applications/DictateKit.app/Contents/MacOS/DictateKit --log-level=debug

# Windows
DictateKit.exe --log-level=debug
```
...
```
DICTATEKIT_LOG_LEVEL=debug
```

**Env file locations:**

- macOS: `~/Library/Application Support/DictateKit/.env`
- Windows: `%APPDATA%\DictateKit\.env`
- Linux: `~/.config/DictateKit/.env`

## Log File Locations

- **macOS**: `~/Library/Application Support/DictateKit/logs/debug-*.log`
- **Windows**: `%APPDATA%\DictateKit\logs\debug-*.log`
- **Linux**: `~/.config/DictateKit/logs/debug-*.log`
...
- Remove `DICTATEKIT_LOG_LEVEL` from `.env`
