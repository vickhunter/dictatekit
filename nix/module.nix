...
let
  cfg = config.programs.dictatekit;
in
{
  options.programs.dictatekit = {
    enable = lib.mkEnableOption "DictateKit voice dictation app with Wayland auto-paste support";
    ...
    users = lib.mkOption {
      ...
      description = ''
        ...
        ydotool group (to reach the ydotoold socket) and the uinput group (DictateKit's
  ...
  config = lib.mkIf cfg.enable {
    ...
    # do this, and DictateKit's linux-fast-paste --uinput needs /dev/uinput directly.
