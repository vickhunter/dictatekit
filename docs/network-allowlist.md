...
Outbound hosts the DictateKit desktop app contacts. For firewall, proxy, and
...
Contacted by every install using DictateKit Cloud (the default after
onboarding).
...
DictateKit Cloud routes streaming sessions through one of three providers.
...
| `dictatekit.com`        | HTTPS    | 443  | OAuth desktop callback redirect (`/auth/desktop-callback`). |
